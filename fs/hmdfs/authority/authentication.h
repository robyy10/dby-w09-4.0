...
{
	if (cred->fsuid.val != multiuser_get_uid(user_id, AID_SYSTEM) ||
	    cred->fsgid.val != multiuser_get_uid(user_id, AID_SYSTEM))
		hmdfs_warning("uid %u gid %u", cred->fsuid.val, cred->fsgid.val);
}
...
static inline
void hmdfs_check_cred(unsigned int user_id, const struct cred *cred)
{
}
