...
static void *g_hw_adsp_ssr_handle;
static struct notifier_block g_hw_adsp_ssr_nb;
...
static int hw_adsp_ssr_callback(struct notifier_block *nb, unsigned long code, void *data)
{
	switch (code) {
	case SUBSYS_AFTER_POWERUP:
		pr_info("hw_adsp_ssr_callback [SUBSYS_AFTER_POWERUP] event.\n");
		hw_cal_unmap_memory();
		break;
	default:
...
int __init hw_adsp_apr_interface_init(void)
{
	...
	g_hw_adsp_ssr_nb.notifier_call = hw_adsp_ssr_callback;
	g_hw_adsp_ssr_handle = subsys_notif_register_notifier("adsp", &g_hw_adsp_ssr_nb);
...
{
	subsys_notif_unregister_notifier(g_hw_adsp_ssr_handle, &g_hw_adsp_ssr_nb);
	hw_cal_unmap_memory();
